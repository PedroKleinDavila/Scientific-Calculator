...
function EE(){
    if(equalsWasLast){
        if(whereDot(operations)==-1){
            let answer=Number(operations)/(10**(operations.length-1));
            visor.value=String(answer)+"×"+"10"+"^"+String(operations.length-1);
            equalsWasLast=false;
        }else{
            let dotIndex=whereDot(operations);
            let answer=Number(operations)/(10**(dotIndex-1));
            visor.value=String(answer)+"×"+"10"+"^"+String(aux3-1);;
            equalsWasLast=false;
        }
    }
}
...
function op(i){
    switch(i){
        case 0:
            ...
            break;
        case 1:
            if(!openParenthesis){if(!operationWasLast||equalsWasLast){
                reducingOp();
                operations+="+";
                operationVisor.value=operations;
                operationWasLast=true;
            }}
            else{if(!operationWasLast||equalsWasLast){
                operations+="+";
                operationVisor.value=operations;
                operationWasLast=true;
                equalsWasLast=false;
